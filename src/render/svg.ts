...
const LEVEL_RING_CIRCUMFERENCE =
  2 *
  Math.PI *
  LEVEL_RING_RADIUS;
...
export async function renderSkyBlockCard(
  ...
  setFill(
    document,
    "player-name",
    theme.minecraft[data.rankColor],
  );

  const nameWidth =
    measureNametagText(data.username);
  ...
  const gap =
    emblemText
      ? measureNametagText(" ") / 3
      : 0;

  const emblemWidth =
    measureNametagText(emblemText);

  const nametagWidth = gap + emblemWidth + gap + nameWidth + gap + emblemWidth + gap;
  const nametagX = 106 - nametagWidth / 2 + gap;

  setX(
    document,
    "player-nametag-background",
    nametagX,
  );
  ...
  setText(
    document,
    "purse",
    formatInteger(data.purse),
  );

  setText(
    document,
    "bank",
    formatInteger(data.bank),
  );
  ...
  for (
    const [slayerIndex, slayer]
    of data.slayers.entries()
  ) {
    setText(
      document,
      `${slayer.key}-level`,
...
function animateRing(
  ...
  const filled =
    ...
    clamp01(progress);
  const empty =
    LEVEL_RING_CIRCUMFERENCE - filled;
...
function animateBar(
  ...
  appendAnimation(
    ...
    id,
    {
      ...
      dur: `${duration}s`,
      values:
        `0;0;${Math.max(0, width).toFixed(2)}`,
      keyTimes: `0;${start.toFixed(3)};1`,
      fill: "freeze",
      calcMode: "spline",
      keySplines:
