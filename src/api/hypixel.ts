...
interface HypixelSkillsResponse {
  success: boolean;
  cause?: string;
  skills?: Record<string, HypixelSkillResource>;
}
...
export class HypixelClient {
  public constructor(private readonly apiKey: string) {
    if (!apiKey) {
      throw new Error("HYPIXEL_API_KEY is missing.");
    }
  }

  public async getProfiles(uuid: string): Promise<HypixelProfile[]> {
    const body = await this.request<HypixelProfilesResponse>(
    ...
    if (!body.profiles?.length) {
      throw new Error("The player has no accessible SkyBlock profiles.");
    }
  ...
  > {
    const body = await this.request<HypixelSkillsResponse>(
  ...
  public async getPlayer(uuid: string): Promise<HypixelPlayer | null> {
    const body = await this.request<HypixelPlayerResponse>("/v2/player", {
      uuid,
    });
  ...
  private async request<T extends HypixelApiResponse>(
    path: string,
    parameters: Record<string, string>,
  ): Promise<T> {
    const url = new URL(`https://api.hypixel.net${path}`);
    ...
    if (!body.success) {
      throw new Error(body.cause ?? "Hypixel API request failed.");
    }
...
export function selectProfile(
  ...
  if (requestedName) {
    ...
    const profile = profiles.find(
      (candidate) => candidate.cute_name.toLowerCase() === requested,
    );

    if (!profile) {
      throw new Error(`SkyBlock profile "${requestedName}" was not found.`);
    }
  ...
  if (!first) {
    throw new Error("No SkyBlock profiles were returned.");
  }
